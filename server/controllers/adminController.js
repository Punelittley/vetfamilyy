...
exports.getAllDoctors = async (req, res) => {
  try {
    const doctors = await User.findAll({
      where: { role: 'doctor' },
      attributes: ['id', 'fullName', 'email', 'phone', 'gender', 'birthDate', 'isActive', 'createdAt', 'avatarUrl'],
...
exports.getDoctorById = async (req, res) => {
  try {
    const doctor = await User.findByPk(req.params.id, {
      attributes: ['id', 'fullName', 'email', 'phone', 'gender', 'birthDate', 'role', 'isActive', 'createdAt', 'avatarUrl']
    });
...
exports.updatePatient = async (req, res) => {
  try {
    ...
    await patient.update({
      ...
      ownerName: ownerName !== undefined ? ownerName : patient.ownerName,
      ownerPhone: ownerPhone !== undefined ? ownerPhone : patient.ownerPhone,
    ...
    const patients = await Patient.findAll({
      where: {
        [Op.or]: [
          ...
          { ownerName: { [Op.like]: likeQuery } },
          { ownerPhone: { [Op.like]: likeQuery } }
        ]
      },
      include: [
        {
...
exports.getAdminReports = async (req, res) => {
  try {
    const { Visit, Patient, User } = require('../models');
    const { Op } = require('sequelize');
    ...
    const byDoctor = await User.findAll({
      where: { role: 'doctor' },
      attributes: ['id', 'fullName', 'email'],
      include: [
        {
    ...
    const doctorStats = await Promise.all(byDoctor.map(async (doctor) => {
