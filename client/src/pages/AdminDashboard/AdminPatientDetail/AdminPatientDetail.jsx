...
const AdminPatientDetail = () => {
  ...
  const fetchPatientData = async () => {
    try {
      ...
      try {
        const visitsRes = await axiosInstance.get(`/visits/patient/${id}`);
        if (visitsRes.data.success) {
          setVisits(visitsRes.data.visits);
        }
      } catch (visitsError) {
        console.warn('⚠️ Не удалось загрузить визиты:', visitsError.message);
        setVisits([]);
      }
    } catch (error) {
    ...
    const printContent = `
      <html>
        ...
        <body>
          ...
          <div class="section">
            <h2>Диагноз:</h2>
            <p>${visits[0]?.diagnosis?.[0]?.description || 'Не установлен'}</p>
          </div>
              ...
              <div className={styles.careGrid}>
                <div className={styles.careItem}>
                  ...
                  <span className={styles.value}>
                    {patient.nextVisitDate && patient.nextVisitDate !== 'null'
                      ? new Date(patient.nextVisitDate).toLocaleDateString('ru-RU')
                      : 'Не назначена'}
                  </span>
                </div>
                <div className={styles.careItem}>
                  ...
                  <span className={styles.value}>
                    {visits[0]?.diagnosis?.[0]?.description || 'Не установлен'}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className={styles.addVisitForm}>
                ...
                <div className={styles.formGroup}>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setVisitForm({ ...visitForm, diagnosis: e.target.value.slice(0, 50) })}
                    rows="2"
                    maxLength="50"
                    placeholder="Диагноз..."
                  />
